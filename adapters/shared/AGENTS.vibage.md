...
3. CONFIRM OK → **vibage-locate** (runs `assert_gate`; nested dig + dual reports).
4. Paste NEW-CHAT / unclear install → **vibage-bootstrap** (hands off to init) or **vibage-init**.
5. OPC hub doc hygiene (SelfAutoBuz) → **docs-hygiene**, not vibage-locate.
...
Or probe `~/.cursor/skills` → `~/.claude/skills` → `~/.agents/skills` for `vibage-init` then `vibage-locate` (realpath → dirname/dirname).
