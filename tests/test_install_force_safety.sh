...
mkdir -p "$PROJ/vibage-locate"
echo keep > "$PROJ/vibage-locate/KEEP.txt"
bash "$ROOT/scripts/install.sh" --project-skills="$TMP_WS" --force
[[ -f "$PROJ/vibage-locate/KEEP.txt" ]] || { echo "FAIL: real dir deleted under --force"; exit 1; }
[[ ! -L "$PROJ/vibage-locate" ]] || { echo "FAIL: real dir became symlink unexpectedly"; exit 1; }
echo "OK: real dir preserved under --force"

# Foreign symlink must survive --force (manual rm path)
rm -rf "$PROJ/vibage-locate"
mkdir -p "$TMP_WS/foreign-skill"
echo foreign > "$TMP_WS/foreign-skill/SKILL.md"
ln -sfn "$TMP_WS/foreign-skill" "$PROJ/vibage-locate"
bash "$ROOT/scripts/install.sh" --project-skills="$TMP_WS" --force
[[ -L "$PROJ/vibage-locate" ]] || { echo "FAIL: foreign symlink removed"; exit 1; }
