...
| **Deliverable** | `VIBAGE-ISSUE-OWNER/LOCATE.md` narrate 掃透 / 立體場景 / dig-ready without holding the token | `verify-report.sh` → `REPORT_TOKEN_LINT_OK` + `COVERAGE_BOX_OK`, `verify-run.sh` mode honesty | **Reachable in principle**, currently partial. Dig coverage is now readable from the report alone (reader-proven); paraphrase is bounded, not blocked |
...
1. **Machine-filled coverage box** — **built** (`scripts/coverage-box.sh`,
   `COVERAGE_BOX_OK`). A required section in both reports, generated by script and
   **re-derived from the hub at verify time**, so a hand-edited box fails. It
   re-implements no gate logic: every fact comes from invoking the existing
   `verify-*` script and parsing its stdout token. Two properties follow:
   - A paraphrase claiming full coverage now sits directly under
     `repos_dug: 0 / 3 · missing-env-config 1 · 掃透: NO`. It is **not blocked** —
     it is **contradicted by its own header**, without the lint understanding any
     language. Paraphrase stops being blocked and starts being pointless.
   - `## Held tokens` must be a **subset** of the box's machine-derived `held:`
     list. An agent can no longer claim a token the workspace does not hold.
2. **Per-finding citation** — not built. Every finding line should carry a path or
   sit under an explicit `## Not verified` heading. Language-independent.

Both are deliverable-surface work; neither claims anything about chat. And note
what the box does **not** do: it does not judge whether a finding is correct, and
`COVERAGE_BOX_SKIPPED` (no derivable hub) is not a pass — the token says so aloud.
