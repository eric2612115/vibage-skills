...
2. **Deepen is not “understood”** — `MAP_DEEPEN_OK` / dossiers never narrate full-understanding, dig-ready-after-install, or Gate A full-sweep (`MATRIX_SWEEP_SUBSTANTIVE_OK`). Locate **ignores deepen-as-auth**.
...
| Dimension fill / repo synthesizer wave | **W3a dimension-fill On-tree** (`DIMENSION_FILL_W3A_P0_OK` + `DIMENSION_FILL_W3A_P1_OK` + `DIMENSION_FILL_W3A_P2_OK`; P0 search+verify · P1 orchestrator · P2 deepen-migrate brand retire). Phrase: `W3a dimension-fill On-tree ≠ full-sweep ≠ understood ≠ letter B` (∉ Tier-0; script-only Proven ≠ live-panel) |
| Env vacancy ask / configure (W2) | **W2 env-vacancy On-tree ≠ full-sweep ≠ letter B** — `ENV_VACANCY_W2_OK`; design `…-env-vacancy-ask-design.md` |
...
- **Gate A** (narrative): rollup / matrix / full-sweep / scene cover tokens.  
