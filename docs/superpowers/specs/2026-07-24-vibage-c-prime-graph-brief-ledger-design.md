...
| no-missed-scan（） | `ENV_BRANCH_MATRIX_OK` |
| full-environment full-branch full-sweep | `MATRIX_SWEEP_SUBSTANTIVE_OK` only (§2.3; implies matrix OK + all real-env cells proven) |
| multi-domain scene cover | `SCENE_BRIEF_OK` **and** `verify-scene-cover.sh` exit 0 — BRIEF alone ≠ cover; **does not** require matrix |
...
Until matrix complete: may not claim matrix-terminal-state／full-sweep／install→ready. 
...
| `ENV_BRANCH_MATRIX_OK` | Inventory complete, 1:1 rows, every cell `proven` or `failed`, no overflow, env vacancy rules held | ≠ full-sweep slogan; ≠ dig auth |
| `MATRIX_SWEEP_SUBSTANTIVE_OK` | `ENV_BRANCH_MATRIX_OK` **and** discovery set has ≥1 real-env cell **and** every real-env cell is `proven` with non-empty evidence (any real-env `failed` ⇒ not OK) **and** zero `missing-env-config` cells remain **and** `env_vacancy_waiver` never grants this token | alone ≠ full-sweep |
...
**Friend chaos bar — `ENV_BRANCH_MATRIX_OK` (matrix-terminal-state／no-missed-scan; full-sweep `MATRIX_SWEEP_SUBSTANTIVE_OK`):**
...
**Verify (`verify-matrix-substantive.sh`) must check:** `ENV_BRANCH_MATRIX_OK`; ≥1 real-env cell; zero `missing-env-config`; zero `unknown-env`; zero `failed` among real-env cells; every real-env cell `proven` with pointers containing non-empty `path` **and** `quote` (or verifiable `evidence_hash`) matching that cell’s `branch_ref`/`env_id`; `env_vacancy_waiver` never grants full-sweep; print `MATRIX_SWEEP_SUBSTANTIVE_OK`.

| Slogan | Token |
|--------|-------|
| no-missed-scan／ | `ENV_BRANCH_MATRIX_OK` |
| environment branch full-sweep | `MATRIX_SWEEP_SUBSTANTIVE_OK` only (all real-env cells proven — not “mostly failed + one proven”) |
...
**Cover claim “multi-domain scene cover”** iff **all** of:
...
4. Does **not** require matrix full-sweep 
...
|----|----------|------------------------|
| F1 | Friend multi-MS + branches + envs | Nested discovery; bounded sparse matrix + caps; no unknown-env cheat; auto sweep; `ENV_BRANCH_MATRIX_OK` + `MATRIX_SWEEP_SUBSTANTIVE_OK` for full-sweep slogan; sync; `friend-chaos` forbids env vacancy waiver |
...
Matrix incomplete ≠ block ticket intake; blocks **matrix-terminal-state／full-sweep** slogans only. 
...
- Incomplete/overflow matrix → tickets OK with disclosure; **no** full-sweep claim. 
