...
| `docs/vibage/dossiers/<id>.md` | Optional dimension-fill stub — cite ledger ids; verify via `verify-dimension-fill.sh` → `DIMENSION_FILL_*` (`MAP_DEEPEN_OK` retired; ≠ Plan-L Graphify; ≠ CONFIRM) |
...
**Honesty:** Graphify CLI is fail-soft (`OK:GRAPHIFY_SKIP`). Map qualify ≠ Architecture Pass ≠ letter B alone. Plan-L Mermaid/Graphify ≠ cost-consented dimension-fill / `DIMENSION_FILL_*` (legacy deepen brand retired).
